...
export default function AuthPage(){
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setMessage('');try{const supabase=createBrowserSupabase();const{error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${window.location.origin}/dashboard`}});if(error)throw error;setMessage('Check your email — your secure UpkeepCue sign-in link is on the way.')}catch(err){setMessage(err instanceof Error?err.message:'Unable to send sign-in link.')}finally{setBusy(false)}}
  return <main className="authShell">
    <Link href="/" className="brandWrap"><div className="brandMark">⌂</div><div className="brand">UpkeepCue</div></Link>
    <section className="card authCard">
      ...
      <div className="divider"/><p className="fine">By continuing, you’re creating or signing into your UpkeepCue account. Your maintenance data is kept separate from other UpkeepCue households.</p>
    </section>
  </main>
}
